...
export default function Annotate() {
  ...
  const annotationSamples = annotationSampleResponse?.samples ?? [];
  const totalCount = annotationSampleResponse?.count ?? 0;
  ...
  useEffect(() => {
    if (annotationSamples.length > 0 && currentSampleIndex < annotationSamples.length) {
      setCurrentSample(annotationSamples[currentSampleIndex]);
    } else {
  ...
  const handleNextSample = () => {
    ...
    if (currentSampleIndex < annotationSamples.length - 1) {
